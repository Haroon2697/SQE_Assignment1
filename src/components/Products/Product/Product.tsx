...
const Product = ({ product }: IProps) => {
  ...
  const formattedPrice = formatPrice(price, currencyId) || '0.00';
    ...
    <S.Container onKeyUp={handleAddProductWhenEnter} sku={sku} tabIndex={1}>
      ...
      <S.Price>
        <S.Val>
          ...
          <b>{formattedPrice && formattedPrice.length > 3 ? formattedPrice.substring(0, formattedPrice.length - 3) : formattedPrice}</b>
          <span>{formattedPrice && formattedPrice.length > 3 ? formattedPrice.substring(formattedPrice.length - 3) : ''}</span>
        </S.Val>
        {productInstallment}
      </S.Price>
